...
The minimum supported v0.1.0 release is Forgejo `16.0.3+gitea-1.22.0`. Forgejo 16.0.2 is retained only as a comparison baseline, not as a supported deployment target. See [Known limitations](known-limitations.md) before connecting a later version.
...
The access token remains short-lived and is refreshed automatically by the MCP client. `FMCP_OAUTH_REFRESH_TOKEN_TTL_DAYS` is the default authorization duration selected at consent, while `FMCP_OAUTH_REFRESH_TOKEN_MAX_TTL_DAYS` caps the available 1, 7, 30 and 90-day choices. Rotation never extends the selected absolute expiry. Inside the short reuse grace, duplicate client requests receive the exact first replacement pair from a bounded in-memory recovery entry, never independent family branches. A missing entry rejects the duplicate without revoking the successful rotation; an older replay still revokes the complete family. Multi-session clients validated with refresh delays above the 10-second default may use a deployment-specific grace of up to 60 seconds. Use `0` for strict replay handling.
